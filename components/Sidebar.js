...
const Sidebar = () => {
    return (
        <div className='fixed top-0 left-0 h-screen w-20 m-0 flex flex-col  bg-gray-900 text-white'>
            <Link href="/">
                <Image src='/logo.png' width={26} height={35} className='relative flex items-center justify-center 
        mt-6  mx-auto ' />
            </Link>
...
const SidebarIcon = ({ icon, text }) => {
    return (
        <div className='sidebar_icon group'>
            {icon}
            <span className='sidebar_tooltip group-hover:scale-100'>
                {text}
            </span>
        </div>
    )
